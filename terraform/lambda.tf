...
variable "avalia_risco_version" {
}

resource "aws_lambda_function" "avalia_risco_function" {
    function_name   = "AvaliaRiscoFunction"

    s3_bucket       = data.aws_s3_bucket.bucket_lambda.id
    s3_key          = "v${var.avalia_risco_version}/avaliarisco.zip"
